...
fn socket_dir_name(workspace_root: &str) -> PathBuf {
...
fn get_socket_dir(workspace_root: &str) -> PathBuf {
    let dir_path = env::var("NX_SOCKET_DIR")
        .or_else(|_| env::var("NX_DAEMON_SOCKET_DIR"))
        .map(PathBuf::from)
        .unwrap_or_else(|_| socket_dir_name(workspace_root));

    let path = match fs::create_dir_all(&dir_path) {
        Ok(_) => dir_path,
...
pub fn get_full_os_socket_path(workspace_root: &str) -> PathBuf {
    get_socket_dir(workspace_root)
}

pub fn get_full_nx_console_socket_path(workspace_root: &str) -> PathBuf {
    get_socket_dir(workspace_root).join("nx-console.sock")
}
